...
public class TCPClient2 {
	...
	Thread t1;
	ComboBoxFrame2 x;
	HashMap<Integer, ChatWindow2> chats=new HashMap<Integer, ChatWindow2>();
	public TCPClient2() {
		GUI();
	}
	void go(String ss) throws UnknownHostException, IOException{
		...
		this.x = new ComboBoxFrame2(y,this);
		this.x.setVisible(true);
		t1=new Thread(new Runnable() {
			public void run() {
				while (true){
					try {
						for(Map.Entry<Integer, ChatWindow2> mp : chats.entrySet()){
							int id = -1;
							id=mp.getKey();
							ChatWindow2 w=mp.getValue();
							if(w.read){
								s= w.p2.getText();
					...
					try {
						...
						if (ms!=null){
							...
							if(g.length == 2){
								...
								ms=g[1];
								if (!chats.containsKey(id))chats.put(id,new ChatWindow2(abc,id));
								ChatWindow2 w=chats.get(id);
								StyleConstants.setForeground(w.style, Color.red);
								w.setVisible(true);
								w.doc.insertString(w.doc.getLength(),"\n"+ms,w.style); }
							else{
								x.setVisible(false);
								x = new ComboBoxFrame2(ms,abc);
								x.setVisible(true);
							}
						}
					}
...
class ChatWindow2 extends JFrame{
	...
	ChatWindow2(TCPClient2 me,int idd){
		this();
		this.me = me;
		this.otherId=idd;
	}
	ChatWindow2(){
		ChatWindow2 abc = this;
		JPanel p=new JPanel();
		setTitle("WebChat");
		p.setLayout(null);
		setLayout(null);
...
class ComboBoxFrame2 extends JFrame{
	...
	String names2[];
	TCPClient2 t;
	public ComboBoxFrame2(String s,TCPClient2 tt){
		t=tt;
		setLocationRelativeTo(null);
		setSize(200,300);
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				for(Map.Entry<Integer, ChatWindow2> mp : t.chats.entrySet()){
					int id=mp.getKey();
					try {
					...
					public void itemStateChanged( ItemEvent event )
					{
						...
						if ( event.getStateChange() == ItemEvent.SELECTED   ){
							...
							if ( k !=-1 && !t.chats.containsKey(pos[k])){
								t.chats.put(pos[k],new ChatWindow2(t,pos[k]));
							}
						}
					} 
				});
		...
		JLabel myName = new JLabel("Name: " + TCPClient2.nameField.getText());
