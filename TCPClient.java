...
public class TCPClient {
	...
	Thread t1;
	ComboBoxFrame x;
	HashMap<Integer, ChatWindow> chats=new HashMap<Integer, ChatWindow>();
	public TCPClient() {
		GUI();
	}
	void go(String ss) throws UnknownHostException, IOException{
		TCPClient abc = this;
		cl = new Socket("127.0.0.1",6000);
		...
		this.x = new ComboBoxFrame(y,this);
		this.x.setVisible(true);
		t1=new Thread(new Runnable() {
			public void run() {
				while (true){
					try {
						for(Map.Entry<Integer, ChatWindow> mp : chats.entrySet()){
							int id = -1;
							id=mp.getKey();
							ChatWindow w=mp.getValue();
							if(w.read){
								s= w.p2.getText();
					...
					try {
						...
						if (ms!=null){
							...
							if(g.length == 2){
								...
								ms=g[1];
								if (!chats.containsKey(id))chats.put(id,new ChatWindow(abc,id));
								ChatWindow w=chats.get(id);
								StyleConstants.setForeground(w.style, Color.red);
								w.setVisible(true);
								w.doc.insertString(w.doc.getLength(),"\n"+ms,w.style); }
							else{
								x.setVisible(false);
								x = new ComboBoxFrame(ms,abc);
								x.setVisible(true);
							}
						}
					}
...
class ChatWindow extends JFrame{
	...
	ChatWindow(TCPClient me,int idd){
		this();
		this.me = me;
		this.otherId=idd;
	}
...
class ComboBoxFrame extends JFrame{
	...
	String names2[];
	TCPClient t;
	public ComboBoxFrame(String s,TCPClient tt){
		t=tt;
		setLocationRelativeTo(null);
		setSize(200,300);
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				for(Map.Entry<Integer, ChatWindow> mp : t.chats.entrySet()){
					int id=mp.getKey();
					try {
					...
					public void itemStateChanged( ItemEvent event )
					{
						...
						if ( event.getStateChange() == ItemEvent.SELECTED   ){
							...
							if ( k !=-1 && !t.chats.containsKey(pos[k])){
								t.chats.put(pos[k],new ChatWindow(t,pos[k]));
							}
						}
					} 
				});
		...
		JLabel myName = new JLabel("Name: " + TCPClient.nameField.getText());
