...
public class TCPClient4 {
	...
	Thread t1;
	ComboBoxFrame4 x;
	HashMap<Integer, ChatWindow4> chats=new HashMap<Integer, ChatWindow4>();
	public TCPClient4() {
		GUI();
	}
	void go(String ss) throws UnknownHostException, IOException{
		...
		this.x = new ComboBoxFrame4(y,this);
		this.x.setVisible(true);
		t1=new Thread(new Runnable() {
			public void run() {
				while (true){
					try {
						for(Map.Entry<Integer, ChatWindow4> mp : chats.entrySet()){
							int id = -1;
							id=mp.getKey();
							ChatWindow4 w=mp.getValue();
							if(w.read){
								s= w.p2.getText();
					...
					try {
						...
						if (ms!=null){
							...
							if(g.length == 2){
								...
								ms=g[1];
								if (!chats.containsKey(id))chats.put(id,new ChatWindow4(abc,id));
								ChatWindow4 w=chats.get(id);
								StyleConstants.setForeground(w.style, Color.red);
								w.setVisible(true);
								w.doc.insertString(w.doc.getLength(),"\n"+ms,w.style); }
							else{
								x.setVisible(false);
								x = new ComboBoxFrame4(ms,abc);
								x.setVisible(true);
							}
						}
					}
	...
	public static void main(String[] args) throws Exception {
		TCPClient4 t1=new TCPClient4();
	}
}
class ChatWindow4 extends JFrame{
	boolean  read ;
	TCPClient4 me;
	int otherId;
	JTextField p2;
	...
	ChatWindow4(TCPClient4 me,int idd){
		this();
		this.me = me;
		this.otherId=idd;
	}
	ChatWindow4(){
		ChatWindow4 abc = this;
		JPanel p=new JPanel();
		setTitle("WebChat");
		p.setLayout(null);
		setLayout(null);
...
class ComboBoxFrame4 extends JFrame{
	private JComboBox namesJComboBox;
	String [] names;
	int pos [];
	String names2[];
	TCPClient4 t;
	public ComboBoxFrame4(String s,TCPClient4 tt){
		t=tt;
		setLocationRelativeTo(null);
		setSize(200,300);
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				for(Map.Entry<Integer, ChatWindow4> mp : t.chats.entrySet()){
					int id=mp.getKey();
					try {
					...
					public void itemStateChanged( ItemEvent event )
					{
						...
						if ( event.getStateChange() == ItemEvent.SELECTED   ){
							...
							if ( k !=-1 && !t.chats.containsKey(pos[k])){
								t.chats.put(pos[k],new ChatWindow4(t,pos[k]));
							}
						}
					} 
				});
		...
		JLabel myName = new JLabel("Name: " + TCPClient4.nameField.getText());
